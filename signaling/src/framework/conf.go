...
func loadConf(confFile string) (*FrameworkConf, error) {
	...
	conf.httpPort, err = configFile.Int("http", "httpPort")
	if err != nil {
		return nil, err
	}

	conf.httpStaticDir, err = configFile.GetValue("http", "httpStaticDir")
	if err != nil {
		return nil, err
	}

	conf.httpStaticPrefix, err = configFile.GetValue("http", "httpStaticPrefix")
	if err != nil {
		return nil, err
	}

	conf.httpsPort, err = configFile.Int("https", "httpsPort")
	if err != nil {
		return nil, err
	}

	conf.httpsCert, err = configFile.GetValue("https", "httpsCert")
	if err != nil {
		return nil, err
	}

	conf.httpsKey, err = configFile.GetValue("https", "httpsKey")
	if err != nil {
		return nil, err
	}
